...
日付を持つリスト（Works・Articles）と About の表は、左メタ列に `colMeta`（7rem）を
共有する。日付の左端と本文の左端が縦に揃い、索引としての規律はこの1本の縦ラインが
支えているので、セクションごとに幅を変えない。

Apps だけは例外で「名前（可変幅）… 技術（右端）」の目次行にする。アプリ名は長さが
まちまちで固定メタ列に収まらないため。左端は他セクションの日付列と、右端は罫線の
右端と揃うので、縦ラインは崩れない。
